...
class VMwareHostSystemSerializer(serializers.Serializer):
    class _VMwareDatastoreSerializer(serializers.Serializer):
        ...
        url = serializers.CharField(required=True, max_length=1024)
        ...
        vmfsType = serializers.CharField(required=True, max_length=64)
        capacity = serializers.IntegerField(required=True)
        majorVersion = serializers.IntegerField(required=True, allow_null=True)
        ssd = serializers.BooleanField(required=True, allow_null=True)
        local = serializers.BooleanField(required=True, allow_null=True)
        multipleHostAccess = serializers.BooleanField(required=True)

    class _VMwareNetworkSerializer(serializers.Serializer):
        assetId = serializers.IntegerField(required=True)
        moId = serializers.CharField(required=True, max_length=64)
        name = serializers.CharField(required=True, max_length=255)
        vlanId = serializers.CharField(required=True, max_length=15, allow_blank=True)
