...
class Lock:
    def __init__(self, objectClass: any, o: dict, objectName: str = "", *args, **kwargs):
        ...
        if "vmFolderName" in o:
            self.vmFolderName = str(o["vmFolderName"])
    ...
    def isUnlocked(self) -> bool:
        ...
        try:
            ...
            if httpMethod:
                # Check if the API endpoint/s related to objectClass are locked for the asset and vmFolder (if set), on objectName,
                ...
                for oc in self.objectClass:
                    if str(httpMethod) in table:
                        for method, compatibility in table[httpMethod].items():
                            entry = oc+":"+str(method)+":"+self.assetId+":"+self.vmFolderName
    ...
    def lock(self) -> None:
        ...
        # asset and vmFolder (if set), on objectName.
        ...
        try:
            ...
            if httpMethod:
                for oc in self.objectClass:
                    ...
                    entry = oc+":"+str(httpMethod)+":"+self.assetId+":"+self.vmFolderName
                    c = cache.get(entry)
    ...
    def release(self) -> None:
        # Release the lock for objectName for HTTP method/asset/vmFolder.
