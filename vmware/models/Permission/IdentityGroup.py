    ...
    def list(showPrivileges: bool = False) -> dict:
        ...
        try:
            c.execute("SELECT "
                ...
                "IFNULL(GROUP_CONCAT( "
                    "DISTINCT CONCAT(role.role,'::',CONCAT(vmFolder.id_asset,'::',vmFolder.vmFolder)) " 
                    "ORDER BY role.id "
                    "SEPARATOR ',' "
                "), '') AS roles_vmFolder, "

                "IFNULL(GROUP_CONCAT( "
                    "DISTINCT CONCAT(privilege.privilege,'::',vmFolder.id_asset,'::',vmFolder.vmFolder,'::',privilege.propagate_to_all_asset_vmFolders,'::',privilege.propagate_to_all_assets) " 
