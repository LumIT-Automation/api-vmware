...
function System_codeCollect()
{
    ...
    cp -R ../vmware $workingFolderPath/var/www/api
    cp ../license.txt $workingFolderPath/var/www/api

    # Remove development db data.
    if [ -f $workingFolderPath/var/www/api/vmware/sql/vmware.data-development.sql ]; then
        rm -f $workingFolderPath/var/www/api/vmware/sql/vmware.data-development.sql
    fi

    # Remove __pycache__ folders and not-required ones.
    rm -fR $workingFolderPath/var/www/api/vmware/tests
...
function System_codeFilesPermissions()
{
    ...
    chmod +x $workingFolderPath/var/www/api/vmware/sql/migrate.sh
