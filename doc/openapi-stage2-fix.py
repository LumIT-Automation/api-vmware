...
def getBlock(l: list, startIdx: int) -> list:
    ...
    try:
        for i in range(startIdx+1, len(l)):
            ...
            if re.match('\s+/(vmware|stage2)/(.*/)?:', l[i]) or re.match('^[a-z]+:', l[i]):
                break
            else:
                b.append(l[i])
...
def getBlocksIndexes(l: list) -> (int, int):
    ...
    try:
        for i in range(len(l)):
            if re.match('\s+/(vmware|stage2)/(.*/)?:', l[i]):  # line delimiting of a block or end last block.
                if not start:
                    start = i
                else:
                    blocks.append({
...
# Identify the urls parameters, using the VMwareUrls.py file.
